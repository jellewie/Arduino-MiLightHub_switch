...
class COTA {
  public:
    String UpdateWebpage = "<a href=\"https://github.com/jellewie/Arduino-MiLightHub_switch\">Github.com/jellewie/Arduino-MiLightHub_switch</a>";
    bool Enabled;
    void handle_uploadPage() {
#ifdef OTA_SerialEnabled
      Serial.println("OTA_handle_UploadPage, enabled=" + String(Enabled));
#endif //OTA_SerialEnabled
      if (!Enabled) return;                                     //If OTA is disabled, stop here and do not respond
    ...
    void handle_update() {
#ifdef OTA_SerialEnabled
      Serial.printf("OTA: Update, enabled=" + String(Enabled));
#endif //OTA_SerialEnabled
      if (!Enabled) return;                                     //If OTA is disabled, stop here and do not respond
